...
@test "it must get the repository name from the given source location w/o path" {
  run provider:github:repository_from_location "github.com:lktslionel/mock-repo"  3>/dev/null
  
  assert_equal "${output}" "lktslionel/mock-repo"
}

@test "it must get the repository name from the given source location w/ path" {
  run provider:github:repository_from_location "github.com:lktslionel/mock-repo:docs"  3>/dev/null
...
@test "it must download a directory from a repository at a given location w/ path to a given destination" {
  ...
  run provider:github:download_from_location "github.com:lktslionel/mock-repo:docs" "dst/default/dir" 3>/dev/null
  assert_file_exists "dst/default/dir/docs-README.md"
}
